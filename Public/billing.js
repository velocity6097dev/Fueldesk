...
// Grab the vehicle input if it exists in your HTML
...
printBtn.addEventListener('click', async () => { 
    ...
    const amount = mode === 'AMOUNT' ? inputVal : +(inputVal * rate).toFixed(2); 
    const timeMode = timeModeSelect.value; 
    const vehicle_no = vehicleInput ? vehicleInput.value.trim() : '';
    ...
    const { data: inserted, error: insertError } = await window.sb 
        ...
        .insert([{ 
            ...
            attendant_id: window.currentSession.user.id, 
            attendant_username: window.currentProfile.username
            // Removed vehicle_no from here so it doesn't go to the database
        }]) 
        .select() 
        .single(); 
    ...
    // Generate a formatted 17-digit transaction ID based on the DB ID
    const generatedTxnId = `000000003${String(inserted.id || inserted.receipt_no || '0').padStart(8, '0')}`;
    ...
    document.getElementById('thermal-receipt').innerHTML = template.render({ 
        ...
        vehicleNo: vehicle_no, // Still passing it here so it prints on the receipt
        productLabel: PRODUCT_LABELS[product] || product, 
        density: density, 
        presetTypeLabel: mode === 'VOLUME' ? 'Volume' : 'Amount', 
        rate: rate.toFixed(2), 
    ...
    window.print(); 
    inputValue.value = ''; 
    if (vehicleInput) vehicleInput.value = ''; // Clear vehicle input after print
}); 

(async function init() { 
    // Maintains your logic regarding STATION_STAFF validation
    const profile = await FuelDeskAuth.requireSession('STATION_STAFF'); 
    if (!profile) return; 
