window.BillTemplates.register({
    ...
    render(data) {
        ...
        return `
            ${logoBlock}
            <div class="text-center" style="font-size:13px;letter-spacing:1px;margin-bottom:2px;">Welcomes You</div>
            ${formattedBlock(s.name, 'font-size:12px;', 'center')}
            ${s.address ? formattedBlock(s.address, 'font-size:10px;') : ''}
            ...
            <div class="grid-3-col">
                ...
                <span>FIP No.</span><span>:</span><span class="text-right">${escapeHtml(data.fpId)}</span>
                <span>Nozzle No.</span><span>:</span><span class="text-right">${escapeHtml(data.nozzleNo)}</span>
